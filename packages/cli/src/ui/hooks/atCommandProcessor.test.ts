...
describe('handleAtCommand', () => {
  ...
  it('should process a file path case-insensitively', async () => {
    ...
    const queryPath = path.join('path', 'to', 'myfile.txt');
    const resolvedQueryPath = path.resolve(testRootDir, queryPath);
    ...
    expect(mockOnDebugMessage).toHaveBeenCalledWith(
      `Path ${queryPath} resolved to filex: ${resolvedQueryPath}`,
    );
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${queryPath}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${resolvedQueryPath}:\n` },
        { text: fileContent },
        { text: '\n--- End of content ---' },
      ],
      shouldProceed: true,
